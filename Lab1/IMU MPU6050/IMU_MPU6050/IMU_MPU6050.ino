...
double Pitch_angle, Roll_angle, Pitch_accel, Roll_accel,cYaw_angle;
...
void SensorFusion() {
  ...
  //超過 4096*4096*2 = 2G 表示加速規不準，不採納其讀值
  if (abs(g_total) < 33554432) {
    Pitch_accel = atan2(accx, sqrt(accy * accy + accz * accz)) * RAD_TO_DAG;
